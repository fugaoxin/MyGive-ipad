...
- (void)viewDidLoad
{
    ...
    if(self.rootViewController){
        ...
        self.rootViewController.navigationBar.barTintColor= [UIColor colorWithHexString:@"#F1F2F3"];
        [self.containerBackgroundView addSubview:self.rootViewController.navigationBar];
        [self.rootViewController.navigationBar showLeftItemWithImage:@"common_close"];
...
-(void)setShowLeftItem:(BOOL)showLeftItem{
    ...
    if (showLeftItem) {
        [self.rootViewController.navigationBar cancelLeftItem];
    }
}
...
-(void)popToRootYQViewControllerAnimated:(BOOL)animated{
    if(self.viewControllers.count>1){
        ...
        YQViewController *currentViewController = self.viewControllers.lastObject;
        YQViewController *rootViewController = self.viewControllers[0];
        
        YQNavigationBar *currentNavigationBar = currentViewController.navigationBar;
        YQNavigationBar *toNavigationBar = rootViewController.navigationBar;
        
        [currentViewController willMoveToParentViewController:nil];
        [self addChildViewController:rootViewController];
        [self.containerView addSubview:rootViewController.view];
        rootViewController.view.frame = CGRectMake(self.size.width/2, 0, self.size.width, self.containerView.bounds.size.height);
        [self.containerView sendSubviewToBack:rootViewController.view];
        
        toNavigationBar.title = rootViewController.title;
        toNavigationBar.hidden = NO;
        ...
        if(animated){
            ...
            toNavigationBar.alpha = 0.0f;
            rootViewController.view.frame = CGRectMake(-self.size.width/2, 0, self.size.width, self.containerView.bounds.size.height);
            [self transitionFromViewController:currentViewController toViewController:rootViewController duration:0.2 options:UIViewAnimationOptionCurveLinear animations:^{
                toNavigationBar.alpha = 1.0f;
                currentNavigationBar.alpha = 0.0f;
                rootViewController.view.frame = self.containerView.bounds;
