...
@interface EGRegisterAllViewController ()<UIPickerViewDelegate,UIActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,UITextFieldDelegate>
{
    ...
    BOOL isHavebase64AvatarPerson;//
    BOOL isHavebase64AvatarOrganiza;//
...
@implementation EGRegisterAllViewController
...
- (void)viewDidLoad {
    ...
    [_personBtn setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
    [_personBtn setTitleColor:[UIColor colorWithHexString:@"#7d7d7d"] forState:UIControlStateNormal];
    [_personBtn setTitleColor:[UIColor colorWithHexString:@"#69318f"] forState:UIControlStateSelected];
    
    [_organizaBtn setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
    [_organizaBtn setTitleColor:[UIColor colorWithHexString:@"#7d7d7d"] forState:UIControlStateNormal];
    [_organizaBtn setTitleColor:[UIColor colorWithHexString:@"#69318f"] forState:UIControlStateSelected];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshUI) name:@"RefreshUI_kevin" object:nil];
...
-(void)loadFBandWBdata
{
    ...
    if ([standardUserDefaults objectForKey:@"FACEBOOK_REG_REQ"] != nil && [[standardUserDefaults objectForKey:@"FACEBOOK_REG_REQ"] isEqualToString:@"1"])
    {
        ...
        if ([EGVerifyTool isVerificatioZhongWen:[_fbdata objectForKey:@"last_name"]]){
            _lastNameCh.text = [_fbdata objectForKey:@"last_name"];
            
        }else{
            _lastNameEn.text = [_fbdata objectForKey:@"last_name"];
        }
        if ([EGVerifyTool isVerificatioZhongWen:[_fbdata objectForKey:@"first_name"]]) {
        ...
        if (_personBtn.selected) {
            if (!isHavebase64AvatarPerson) {
                ...
                base64AvatarPerson = [standardUserDefaults objectForKey:@"FACEBOOK_PICTURE"];
            }
            
        }else{
            if (!isHavebase64AvatarOrganiza) {
                self.userImgeView.image = [UIImage imageWithData:[[NSData alloc] initWithBase64EncodedString:[standardUserDefaults objectForKey:@"FACEBOOK_PICTURE"] options:NSDataBase64DecodingIgnoreUnknownCharacters]];
                base64AvatarOrganiza = [standardUserDefaults objectForKey:@"FACEBOOK_PICTURE"];
            }
    ...
    } else if ([standardUserDefaults objectForKey:@"WEIBO_REG_REQ"] != nil && [[standardUserDefaults objectForKey:@"WEIBO_REG_REQ"] isEqualToString:@"1"])
    {
        ...
        if (_personBtn.selected) {
            if (!isHavebase64AvatarPerson) {
                ...
                base64AvatarPerson = [standardUserDefaults objectForKey:@"WEIBO_PICTURE"];
            }
            
        }else{
            if (!isHavebase64AvatarOrganiza) {
                self.userImgeView.image = [UIImage imageWithData:[[NSData alloc] initWithBase64EncodedString:[standardUserDefaults objectForKey:@"WEIBO_PICTURE"] options:NSDataBase64DecodingIgnoreUnknownCharacters]];
                base64AvatarOrganiza = [standardUserDefaults objectForKey:@"WEIBO_PICTURE"];
            }
...
-(void)setPersonUI{
    ...
    [self.userImgeView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(98, 98));//198px
    }];
    //用户
...
//        make.leading.mas_equalTo(60);
    ...
    self.confirmPW.keyboardType = UIKeyboardTypeASCIICapable;
...
//        make.trailing.equalTo(_contantPersonView).offset(-60);
    ...
    [self.segment_M mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_pwMessageLab.mas_bottom).offset(13);
    ...
    self.lastNameEn.keyboardType = UIKeyboardTypeASCIICapable;
    ...
    self.nameEn.keyboardType = UIKeyboardTypeASCIICapable;
...
-(void)setOrganizaUI{
    ...
    self.organizaNameEn.keyboardType = UIKeyboardTypeASCIICapable;
    ...
    self.otherOrganizaNumber.keyboardType = UIKeyboardTypeNumberPad;
    ...
    [line1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_organizaNumber.mas_bottom).offset(30+20+30+40);
        make.leading.equalTo(_organizaNumber);
        make.trailing.equalTo(_organizaNumber);
        make.height.mas_equalTo(1);
    }];
    ...
    self.telCodeField = [self createTextFild];
    self.telCodeField.keyboardType = UIKeyboardTypeNumberPad;
...
-(void)iconImageTapAction{
    ...
    //在0＊0区域显示这个Popover
    [photoBtnActionSheet showFromRect:CGRectMake(self.userImgeView.center.x, CGRectGetMaxY(self.userImgeView.frame), 0, 0) inView:self.view animated:YES];
    ...
    if (buttonIndex == 0){
        @try {
            if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
                ...
                [popover presentPopoverFromRect:CGRectMake(self.userImgeView.center.x, CGRectGetMaxY(self.userImgeView.frame), 0, 0) inView:self.view permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (_personBtn.selected) {
        ...
        isHavebase64AvatarOrganiza = YES;
...
- (IBAction)receiveViewButtonAction:(UIButton*)sender
{
    ...
    if (sender) {
        if(sender.tag == 7) {
            ...
            UIButton*  receiveViewButton= _receiveBtns.lastObject;
            if(receiveViewButton.tag == 7 && receiveViewButton.selected){
                receiveViewButton.selected = NO;
                [_receiveSelArr removeObject:receiveViewButton];
...
- (IBAction)organizationTypeAction:(UISegmentedControl*)sender {
    ...
    if (sender.selectedSegmentIndex == 3) {//其他
        ...
        [line1 mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_organizaNumber.mas_bottom).offset(20+(30+30+10));//otherOrganizaNumber
        }];
...
-(void)commitBtnActionPerson{
    ...
    //姓名
    else if([self nameStrIsTruePerson]){
...
-(void)commitBtnActionOrganiza{
    ...
    //
    else if (_organizaNameCh.text.length == 0 && _organizaNameEn.text.length == 0){
...
-(void)showMessageVCWithTitle:(NSString*)title type:(NSString*)type message:(NSString*)message messageButton:(NSString*)btnTitle
{
    ...
    [root setMessageAction:^(EGRegisterAlertViewController *vc) {
        if ([type isEqualToString:@"message_registerSuccess"]) {
            ...
            [def synchronize];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"dismRegisterVC_kevin" object:nil];
        }
    }];
...
-(void)refreshUI
{
    ...
    //    [self getAreaArr];
    [self setUIString];
    [_pickerViewPopupBelongTo reloadAllComponents];
    [_pickerViewPopupBusinessType reloadAllComponents];
    //    [self keyboardWillHide_register];
    _belongToType.text = @"";
    if (_segment_OrganizationType.selectedSegmentIndex == 3) {//其他
        _organizaNumber.text = HKLocalizedString(@"不适用");
