...
- (void)viewDidLoad {
    ...
    contentView.backgroundColor = [UIColor colorWithHexString:@"#F1F2F4"];
...
//        make.edges.equalTo(self.view).with.insets(UIEdgeInsetsMake(0, 0, 0, 0));//top left bottom right.
    ...
    [_noteScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(contentView).with.insets(UIEdgeInsetsMake(40, 0, 60, 0));
    }];
    ...
    [cancelButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_noteScrollView.mas_bottom).offset(10);
